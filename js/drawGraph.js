...
async function drawCurve(container, startx, starty, endx, endy, color) {
  // Offset to prevent path overlap at circle edges
  // Using 8px to account for commitHeadDot radius (7px) + margin
  const circleOffset = 8;
  var adjustedStartY = starty + circleOffset;
  var adjustedEndY = endy - circleOffset;

  var firstLineEndY = adjustedStartY + ((adjustedEndY - adjustedStartY - 40) / 2);
  var secondLineStartY = firstLineEndY + 40;
  container.innerHTML += '<path d = "M ' + startx + ' ' + adjustedStartY + ' L ' + startx + ' ' + firstLineEndY + ' C ' + startx + ' ' + (parseInt(firstLineEndY) + 20) + ' , ' + endx + ' ' + (parseInt(firstLineEndY) + 20) + ' , ' + endx + ' ' + (parseInt(firstLineEndY) + 40) + ' L ' + endx + ' ' + adjustedEndY + '" stroke="' + color + '" stroke-width="1" fill = "#00000000"/>';
}
...
async function drawDottedLine(container, startx, starty, color) {
  // Offset to start below the circle edge (consistent with drawCurve)
  const circleOffset = 8;
  var adjustedStartY = starty + circleOffset;
